...
class SignalAnalyzer {
  ...
  async generateSignals(type) {
    ...
    try {
      ...
      this.cache[type] = {
        signals: signals.map(s => ({ ...s, pairDisplay: 'USD/INR' })),
        fetchedAt: Date.now()
      };

      const bot = timezoneConverter.getCurrentBotTime();
      console.log(`📊 INR ${type}: ${signals.length} signals | Bot time (UTC+6): ${String(bot.hour).padStart(2,'0')}:${String(bot.minute).padStart(2,'0')}:${String(bot.second).padStart(2,'0')}`);
